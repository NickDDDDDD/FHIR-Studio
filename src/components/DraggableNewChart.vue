    ...
    >
      ...
      <div v-if="isPointSelected" title="Point Information">
        <el-row gutter="20">
          ...
          <el-col :span="12">
            <el-input
              v-model="computedSelectedPointY"
              placeholder="Y coordinate"
              :suffix-icon="this.unit"
            >
        ...
        <el-row style="margin-top: 20px">
          <el-col :span="12">
            <el-button
              type="primary"
              @click="saveUpdatedPoint"
              style="width: 100%"
              >Save</el-button
            >
          </el-col>
          <el-col :span="12">
            <el-button type="danger" @click="onPointDelete" style="width: 100%"
              >Delete</el-button
            >
          </el-col>
        </el-row>
      ...
      <div>
        <el-button type="primary" @click="onJsonSend" style="width: 100%"
          >Generate JSON</el-button
        >
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    initChart() {
      ...
      if (isBlank === "false") {
        ...
        if (numberOfPoints == 0) {
          ...
          this.step =
            dateOfStep.getHours() * 60 * 60 * 1000 +
            dateOfStep.getMinutes() * 60 * 1000 +
            dateOfStep.getSeconds() * 1000;
          data = this.generateStepData(timeRange, trend, rangeFrom, rangeTo);
        } else {
          this.dataType = selectedDataType;
          console.log("Data Type : " + this.dataType);
          data = this.generateNumData(
            timeRange,
            trend,
            rangeFrom,
            rangeTo,
        ...
        const recommandNum = [
          Number(recommandRange[0]),
          Number(recommandRange[1]),
        ];
        this.data = data;
        this.unit = unit;
        option = {
    ...
    generateStepData(timeRange, trend, rangeFrom, rangeTo) {
      ...
      if (trend === "up") {
        ...
        currentValue =
          rangeFromNum + Math.random() * (rangeToNum - rangeFromNum);
      }
    ...
    generateNumData(timeRange, trend, rangeFrom, rangeTo, numberOfPoints) {
      ...
      for (let i = 0; i < numberOfPoints; i++) {
        ...
        const timestamp =
          startTime + i * averageTimeInterval + randomTimeInterval;

        let value;
        if (trend === "up") {
          value =
            currentValue + Math.random() * rangeDelta * randomTimeInterval;
          currentValue = Math.min(value, rangeToNum);
        } else if (trend === "down") {
          value =
            currentValue - Math.random() * rangeDelta * randomTimeInterval;
    ...
    handleClick(event) {
      ...
      if (!this.inNode && this.mode === "edit") {
        const pointInPixel = [event.offsetX, event.offsetY];
        const dataIndex = this.chartInstance.convertFromPixel(
          "grid",
          pointInPixel
        );
        if (this.chartInstance.containPixel("grid", pointInPixel)) {
    ...
    saveUpdatedPoint() {
      ...
      this.data[this.selectedPointIndex] = [
    ...
    onPointDragging(dataIndex, event) {
      ...
      const rightNeighbor =
        dataIndex < this.data.length - 1 ? this.data[dataIndex + 1] : null;
    ...
    onJsonSend() {
      console.log("Data Type onJson Send: " + this.dataType);
      const json = this.data.map((dataPoint) =>
        this.dataToObservation(this.dataType, dataPoint[0], dataPoint[1])
      );

      const client = ref(null);
      //just a demo for sending resource, might need to send ownerId and ownerType as well in the later process of development
      FHIR.oauth2
        .ready()
        .then((fhirClientInstance) => {
          client.value = fhirClientInstance;
          for (let i = 0; i < json.length; i++) {
            const result = client.value.create(json[i]);
            console.log(result);
          }
        })
        .catch((error) => {
    ...
    dataToObservation(type, timestamp, value) {
      ...
      switch (type) {
        case "heartRateModel":
          ...
          break;
        case "oxygenSaturationModel":
          console.log("template: ox");
          template = oxygenSaturationTemplate;
          break;
        case "bloodPressureModel":
      ...
      // update patient ID
      if (this.patientId) {
        observation.subject.reference = `Patient/${this.patientId}`;
      }
